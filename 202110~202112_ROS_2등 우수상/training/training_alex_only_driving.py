...
TEST_RATE = 0.2
LEARNING_RATE = 1e-3
# down_list = os.listdir(DATAS_UP_PATH)
down_list = os.listdir(DATAS_DOWN_PATH)
...
for i in range(len(down_list)):
    if down_list[i][-4:] != '.jpg':continue
    follow, to_position, line_num, loaded, position_bool, people = down_list[i].split('_')[0]
    x = down_list[i].split('_')[1]
    y = down_list[i].split('_')[2]
    ...
    x, y = float(x), float(y)
    # img_up = cv2.imread(DATAS_UP_PATH+down_list[0]).reshape(3,224,224)
    img_down = cv2.imread(DATAS_DOWN_PATH+down_list[0]).reshape(3,224,224)
    input_data = (follow, to_position, line_num, loaded, img_down/255.)
    input_label = (x, y)
...
class AutoDrive(torch.nn.Module):
    def __init__(self):
        super().__init__()
        self.output_classes=2 # position_bool, people, x, y
        self.unit_output = 200
    ...
    def forward(self, follow, to_position, line_num, loaded, down_img): # follow / to_position / line_num / loaded
        ...
        x = torch.cat([follow,to_position, line_num, loaded, x_down], dim=-1)
...
for epoch in range(NUM_EPOCHS):
    ...
    for (follow, to_position, line_num, loaded, down_img),(x, y) in train_loader:
        follow = follow.to(device=device); to_position = to_position.to(device=device); line_num = line_num.to(device=device); loaded = loaded.to(device=device);down_img = normalize(down_img.float()).to(device=device);
        ...
        optimizer.zero_grad()
        outputs = auto(follow, to_position, line_num, loaded,down_img.float())
    ...
    with torch.no_grad():
        for (follow, to_position, line_num, loaded, down_img),(x, y) in test_loader:
            follow = follow.to(device=device); to_position = to_position.to(device=device); line_num = line_num.to(device=device); loaded = loaded.to(device=device); down_img = down_img.to(device=device);
            x=x.to(device); y=y.to(device);
            x=x.unsqueeze(-1); y=y.unsqueeze(-1);
